...
export default function Login() {
  ...
  const [message, setMessage] = useState(""); // Keep for error messages
  ...
  const handleLogin = async (e) => {
    ...
    const toastId = toast.loading('Logging in...'); // Loading indicator

    try {
      const res = await loginUser(username, password);
      if (res.token) {
        // --- UPDATED (JWT & Removed Flag Alert) ---
        login({ username: res.username, token: res.token });
        toast.success('Login Successful!', { id: toastId });
        // REMOVED alert(`Login Successful! Flag: ${res.flag}`);
        navigate("/");
      } else {
